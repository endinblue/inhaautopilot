...
/** LINESRATIO */
#ifndef LINESRATIO
#define LINESRATIO   0.3
#endif /* !LINESRATIO */

/** LINESRATIO */
#ifndef DISTLINESRATIO
#define DISTLINESRATIO   0.3
#endif /* !LINESRATIO */

/** LINESRATIO */
#ifndef ANGLERATIO
#define ANGLERATIO   0.5
#endif /* !LINESRATIO */

/** LINESRATIO */
#ifndef TRIANGLE
#define TRIANGLE   1.0471666667
#endif /* !LINESRATIO */
    ...
    if( used->data[ list_p->x + list_p->y * used->xsize ] == NOTUSED &&
          ...
          because we are only interested in the exact NOTDEF value */
      {
        ...
        /*
           ...
           points with an offset of (0.5,0.5), that should be added tooutput.
                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  double quant = 0.5;/*2.0;*/       /* Bound to the quantization error on the
                                gradient norm.                                */
...
{
  /* LSD parameters */
  double scale = 0.3;       /* Scale the image by Gaussian filter to 'scale'. */
...
/*
功能：计算线段之间的关系，删除掉不符合要求的线段
n_out:返回线段的总数
out:返回线段的坐标
link:返回线段之间的关系
*/
void RelationBetweenLines(int n_out,double *out, vector<int> &link)
{
	float x10,x11,x12;
	float y10,y11,y12;
	...
	for(int i=0;i<n_out;i++)
	{
		...
		float dist = sqrt((x12-x10)*(x12-x10)+(y12-y10)*(y12-y10));
		length[i] = dist;
	}
	...
	for(int i=0;i<n_out-1;i++)
	{
		...
		float temp1x = x12-x10;
		float temp1y = y12-y10;
		float mag1 = temp1x*temp1x+temp1y*temp1y;
		mag1 = sqrt(mag1);
		...
		for(int j=i+1;j<n_out;j++)
		{
			...
			float temp2x = x22-x20;
			float temp2y = y22-y20;
			float mag2 = temp2x*temp2x+temp2y*temp2y;
			mag2=sqrt(mag2);

			float crossv = temp1x*temp2x+temp1y*temp2y;
			float angle = acos(crossv/(mag1*mag2));

			float mdist = sqrt((x21-x11)*(x21-x11)+(y21-y11)*(y21-y11));
			if(mdist<2.0*length[i])
			{
				Angle.push_back(angle);
			}
		}
		//printf("%d,%d\n",i,Angle.size());
		if(Angle.size()==0){link[i]=0;continue;}
		/*bool flag = false;
		for(int k=0;k<Angle.size();k++)
		{
			if((Angle[k]/TRIANGLE-1)<ANGLERATIO)
			{
				flag = true;			
			}
		}
		if(flag==false)link[i]=0;*/
		Angle.clear();
	}
	delete []length;
}
...
w,h：图像的宽高
*/
void FirstFilterLines(int n_out,double *out, vector<int> &link,int w,int h)
{
	...
	for(int i=0;i<n_out;i++)
	{
		...
		if(dx/dy<WHRATIO||y1>HCUT||y2>HCUT||length[i]>MAXLENGTH||length[i]<MINLENGTH)link[i]=0;		
	}
...
#if 0
		//线段滤波
		IplImage *binImgFil = cvCreateImage(cvSize(X,Y),8,1);
		cvZero(binImgFil);
		float vx = 0;
		float vy = 1;
		float th = 3.1415*25/180.0;
		for(i=0;i<n;i++)
		{
			float x1 = out[7*i+0];
			float y1 = out[7*i+1];
			...
			float cosvalue = ux*vx+uy*vy;
			cosvalue /= sqrt(ux*ux+uy*uy);
			float theta = acos(cosvalue);
			if(theta<th)continue;
			/*if(theta>th)
			{*/
				for(int t=1;t<TotalNum-1;t++)
				{
					float coff = precsion*t;
					tmpx = (1-coff)*x1+coff*x2;
					tmpy = (1-coff)*y1+coff*y2;
					CV_IMAGE_ELEM ( binImgFil, uchar, tmpy, tmpx) = (uchar)255;
				}
			/*}*/
		}
		//cvSaveImage(nameBinFilImg,binImgFil);
		cvReleaseImage(&binImgFil);
#endif