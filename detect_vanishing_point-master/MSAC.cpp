...
//求得__Li：直线端点(x0,y0)(x1,y1)的叉积，可以表示直线y=ax+b+c,a=y0-y1,b=x1-x0,c=x0*y1-x1*y0
//求得__Mi：直线中点（xm，ym）= （x0/2+x1/2, y0/2+y1/2）
//求得__Lengths:某直线长度占所有直线长度总和的比例
...
void MSAC::estimateNIETO(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &set, int set_length, cv::Mat &vp)
{
	...
	/*lm_status_struct *lm_printout_std;*/
...
float MSAC::errorNIETO(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp, std::vector<float> &E, int *CS_counter)
{
	...
	{ 		
		...
		lineSegment.at<float>(2,0) = Li.at<float>(i,1);//2??
...
void MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
	...
	for(unsigned int vpNum=0; vpNum<vps.size(); vpNum++)
	{
		if(vps[vpNum].at<float>(2,0) != 0)
		{
			...
			int v = vpNum;
			char pcBuffer[256];
			if (vps[v].at<float>(0,0) < 0 && vps[v].at<float>(1,0) < 0)//左上
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) left_up", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if (vps[v].at<float>(0,0) < 0 && (vps[v].at<float>(1,0) > 0 && vps[v].at<float>(1,0) < im.rows))//左
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) left", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if (vps[v].at<float>(0,0) < 0 && vps[v].at<float>(1,0) > im.rows)//左下
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) left_down", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if ((vps[v].at<float>(0,0) > 0 && vps[v].at<float>(0,0) < im.cols) && vps[v].at<float>(1,0) < 0)//上
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) up", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if ((vps[v].at<float>(0,0) > 0 && vps[v].at<float>(0,0) < im.cols)
				&& (vps[v].at<float>(1,0) > 0 && vps[v].at<float>(1,0) < im.rows))//中
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) middle", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if ((vps[v].at<float>(0,0) > 0 && vps[v].at<float>(0,0) < im.cols) && vps[v].at<float>(1,0) > im.rows)//下
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) down", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if (vps[v].at<float>(0,0) > im.cols && vps[v].at<float>(1,0) > im.rows)//右下
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) right_down", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if (vps[v].at<float>(0,0) > im.cols && (vps[v].at<float>(1,0) > 0 && vps[v].at<float>(1,0) < im.rows))//右
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) right", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,1.0,Scalar(255,255,255),1,8,false);//在图片中输出字符   
			}
			else if (vps[v].at<float>(0,0) > im.cols && vps[v].at<float>(1,0) > im.rows)//右下
			{
				sprintf(pcBuffer, "VP %d (%.3f, %.3f, %.3f) right_down", v, vps[v].at<float>(0,0), vps[v].at<float>(1,0), vps[v].at<float>(2,0));
				//putText(outputImg,pcBuffer,Point(20,20),1,3.0,Scalar(255,255,255),1,1,8,false);
			}
			putText(im,pcBuffer,Point(50,50+v*50),1,3.0,colors[v], 3,8,false);//在图片中输出字符  
			//fflush(stdout);
			double vpNorm = cv::norm(vps[v]);
			if(fabs(vpNorm - 1) < 0.001)
			{
				putText(im,"(INFINITE)",Point(50,50+v*50),1,3.0,Scalar(255,255,255),3,8,false);//在图片中输出字符
				//fflush(stdout);
			}
