...
namespace blobing
{
	CBlobLabeling::CBlobLabeling(void)
	{
		...
		m_Image			= NULL;
		m_recBlobs		= NULL;
		m_candidateBlobs= NULL;

		m_candidateLabel = (int *) malloc( _DEF_MAX_LABEL * sizeof(int));
		for(int i=0; i<_DEF_MAX_LABEL; i++) 
			m_candidateLabel[i] = -1;
	...
	//조건에 맞지 않는 레이블링을 삭제한다.
	IplImage* CBlobLabeling::DeleteLabeling(IplImage* srcImage)
	{
		return _DeleteLabeling(srcImage, m_nBlobs, m_recBlobs);
	}

	//실제 레이블링을 삭제하는 부분
	IplImage* CBlobLabeling::_DeleteLabeling(IplImage* srcImage, int m_nBlobs, CvRect* m_recBlobs)
	{
		int nWidth = srcImage->width;
		int nHeight = srcImage->height;
		IplImage* temp = cvCreateImage(cvGetSize(srcImage), 8, 1);
		int nNum=0;

		for(int i=0; i<m_nBlobs; i++)
		{
			if((m_recBlobs[i].height<m_recBlobs[i].width)&&
				//(m_recBlobs[i].width<nWidth*0.8)&&
				//(m_recBlobs[i].height<nHeight*0.8)&&
				(m_recBlobs[i].width>nWidth*0.2)&&
				(m_recBlobs[i].height>nHeight*0.1)&&
				((float)m_recBlobs[i].width/(float)m_recBlobs[i].height>1.7)&&
				((float)m_recBlobs[i].width/(float)m_recBlobs[i].height<5.0))
			{
				for(int j=m_recBlobs[i].y; j<m_recBlobs[i].y+m_recBlobs[i].height;j++)
				{
					for(int k=m_recBlobs[i].x; k<m_recBlobs[i].x+m_recBlobs[i].width;k++)
					{
						temp->imageData[j*nWidth+k] = srcImage->imageData[j*nWidth+k];
					}
				}
				m_candidateLabel[nNum] = i;
				nNum++;
			}
		}
	...
	//후보영역을 얻는다.
	CvRect* CBlobLabeling::GetCandidateBlob(CvRect *Blobs)
	{
		return _GetCandidateBlob(Blobs, m_candidateLabel);
	}
	...
	int CBlobLabeling::BlobSmallSizeConstraint(int nWidth, int nHeight)
	{
		m_nBlobs = _BlobSmallSizeConstraint(nWidth, nHeight, m_recBlobs, m_nBlobs);
		return m_nBlobs;
	}

	int CBlobLabeling::_BlobSmallSizeConstraint(int nWidth, int nHeight, CvRect* rect, int nRecNumber)
	{
	...
	int CBlobLabeling::BlobBigSizeConstraint(int nWidth, int nHeight)
	{
		m_nBlobs = _BlobBigSizeConstraint(nWidth, nHeight, m_recBlobs, m_nBlobs);
		return m_nBlobs;
	}

	int CBlobLabeling::_BlobBigSizeConstraint(int nWidth, int nHeight, CvRect* rect, int nRecNumber)
	{
		if(nRecNumber == 0)	return 0;
	...
	//실제 후보영역을 얻는 함수
	CvRect* CBlobLabeling::_GetCandidateBlob(CvRect *Blobs, int *nLabel)
	{
		m_nCandidate = 0;
		...
		for(int i=0; i<m_nBlobs; i++)
		{
			if(nLabel[i]!=-1)
			{
				m_candidateBlobs[m_nCandidate]=Blobs[i];
				m_nCandidate++;
			}
		}

		return m_candidateBlobs;
	}
}